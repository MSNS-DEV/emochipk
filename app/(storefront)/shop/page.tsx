...
function ShopContent() {
  ...
  const FilterContent = () => (
    <div className="space-y-8">
      <div>
        <h4 className="font-semibold text-sm mb-3 uppercase tracking-wider text-muted-foreground">Style</h4>
        <div className="space-y-2">
          {styleCategories.map((cat) => (
            <div key={cat.id} className="flex items-center space-x-2">
              <Checkbox id={`style-${cat.id}`} checked={filters.style === cat.id}
                onCheckedChange={(checked) => { setFilters((f) => ({ ...f, style: checked ? cat.id : undefined })); setPage(1); }} />
              <Label htmlFor={`style-${cat.id}`} className="text-sm cursor-pointer">{cat.emoji} {cat.label}</Label>
            </div>
          ))}
        </div>
      </div>
      <div>
        <h4 className="font-semibold text-sm mb-3 uppercase tracking-wider text-muted-foreground">Collection</h4>
        <div className="space-y-2">
          {genderCategories.map((cat) => (
            <div key={cat.id} className="flex items-center space-x-2">
              <Checkbox id={`cat-${cat.id}`} checked={filters.category === cat.id}
                onCheckedChange={(checked) => { setFilters((f) => ({ ...f, category: checked ? cat.id : undefined })); setPage(1); }} />
      ...
      <div>
        <h4 className="font-semibold text-sm mb-3 uppercase tracking-wider text-muted-foreground">Size (UK)</h4>
        <div className="flex flex-wrap gap-2">
          {allSizes.map((size) => (
            <button key={size} onClick={() => {
              setFilters((f) => ({ ...f, sizes: f.sizes?.includes(size) ? f.sizes.filter((s) => s !== size) : [...(f.sizes ?? []), size] }));
              setPage(1);
            }} className={`h-9 w-9 rounded-md border text-sm font-medium transition-colors ${filters.sizes?.includes(size) ? 'border-amber-500 bg-amber-500 text-white' : 'border-border hover:border-amber-400'}`}>
              {size}
            </button>
          ))}
        </div>
      ...
      <div>
        <h4 className="font-semibold text-sm mb-3 uppercase tracking-wider text-muted-foreground">Color</h4>
        <div className="flex flex-wrap gap-2.5">
          {allColors.map((color) => (
            <button key={color.name} onClick={() => {
              ...
              setPage(1);
            }} className={`h-8 w-8 rounded-full border-2 transition-all ${filters.colors?.includes(color.name) ? 'border-amber-500 ring-2 ring-amber-500 ring-offset-2' : 'border-border hover:scale-110'}`}
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-muted/40 py-8 lg:py-12 border-b">
        <div className="container mx-auto px-4">
          <h1 className="font-serif text-3xl sm:text-4xl font-bold tracking-tight mb-1">{pageTitle}</h1>
        ...
        <div className="lg:grid lg:grid-cols-[240px_1fr] lg:gap-10">
          ...
          <div>
            <div className="flex items-center justify-between gap-4 mb-6 pb-4 border-b">
              <Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
                ...
                <SheetContent side="left" className="w-[300px] sm:w-[350px]">
                  ...
                  <div className="mt-6 overflow-y-auto max-h-[calc(100vh-200px)] pr-2"><FilterContent /></div>
                  <SheetFooter className="mt-6">
                    <Button onClick={() => setMobileFiltersOpen(false)} className="w-full bg-amber-500 hover:bg-amber-600 text-white">
              ...
              <div className="hidden lg:flex items-center gap-2 flex-wrap flex-1">
                {filters.style && <Badge variant="secondary" className="gap-1">
                  {styleCategories.find((s) => s.id === filters.style)?.label}
                  <button onClick={() => setFilters((f) => ({ ...f, style: undefined }))}><X className="h-3 w-3" /></button>
                </Badge>}
                {filters.category && <Badge variant="secondary" className="gap-1">
                  {genderCategories.find((g) => g.id === filters.category)?.label}
                  <button onClick={() => setFilters((f) => ({ ...f, category: undefined }))}><X className="h-3 w-3" /></button>
                </Badge>}
                {filters.sizes?.map((sz) => <Badge key={sz} variant="secondary" className="gap-1">UK {sz}
                  <button onClick={() => setFilters((f) => ({ ...f, sizes: f.sizes?.filter((s) => s !== sz) }))}><X className="h-3 w-3" /></button>
                </Badge>)}
              </div>
                ...
                <div className="hidden sm:flex items-center border border-border rounded-lg overflow-hidden">
                  {[2, 4].map((c) => (
                    <button key={c} onClick={() => setGridCols(c as 2 | 4)}
                      className={`p-2 transition-colors ${gridCols === c ? 'bg-muted' : 'hover:bg-muted/50'}`}>
            ...
            {!isLoading && data && data.items.length > 0 ? (
              <>
                <div className={`grid gap-4 lg:gap-6 ${gridCols === 2 ? 'grid-cols-2' : gridCols === 4 ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-2 lg:grid-cols-3'}`}>
              ...
              <div className="text-center py-20">
                ...
                <Button onClick={clearAll} className="bg-amber-500 hover:bg-amber-600 text-white">Clear Filters</Button>
              </div>
            )}
          </div>
        </div>
