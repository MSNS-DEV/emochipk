...
export default function AdminDashboard() {
  ...
  const { data: products } = api.product.adminList.useQuery({
    page: 1,
    pageSize: 5,
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">
          Executive Mochi — Admin Overview
        </p>
      </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-zinc-500">
                    {p.articleNumber} · {p._count.variants} variants
                  </div>
                </div>
                <div className="text-xs font-medium text-amber-400">
                  PKR{" "}
                  {Number(p.salePrice ?? p.basePrice).toLocaleString("en-PK")}
                </div>
              </a>
            ))}
          </div>
            ...
            {branches?.map((b) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-zinc-500">
                    {b.city} · {b._count.inventory} SKUs · {b._count.orders}{" "}
                    orders
                  </div>
                </div>
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${b.isActive ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                >
